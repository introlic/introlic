 ...
 export default function WhitepaperGallery() {
   ...
   useEffect(() => {
    ...
    const renderAuthorCard = (authorName: string) => {
      ...
      const initials = authorObj.name ? authorObj.name.split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2) : 'A';
            ...
            <div className="flex items-center gap-3">
              {avatarUrl ? (
                ...
                <div className="relative w-12 h-12 rounded-xl bg-gradient-to-br from-[#00a3ff]/20 to-[#00d1ff]/10 border border-[#00a3ff]/30 flex items-center justify-center select-none shadow-[0_4px_12px_rgba(0,163,255,0.15)]">
                  ...
                  <div className="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-[#00a3ff] border-2 border-[#09090a] flex items-center justify-center text-[7px] font-black text-black">
                    {initials}
                  </div>
                </div>
              )}
